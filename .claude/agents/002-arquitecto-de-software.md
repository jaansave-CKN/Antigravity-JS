...
## Vigencia del estado
Antes de fiscalizar contra un hecho que creas saber sobre este proyecto (qué está activo, qué está bloqueado, qué falta), verifica `docs/ARQUITECTURA_AGENTICA_ANTIGRAVITY.md` — es la fuente de verdad viva, ordenada cronológicamente por secciones fechadas (la fecha más reciente prevalece sobre las anteriores). Un veredicto basado en un estado caducado no es una fiscalización real.
