...
1. **Stubs huérfanos** — componentes que renderizan una maqueta fija sin consumir datos reales. Patrón real vigente en este proyecto (corregido 2026-08-16, cierre de §0-AJ.5 — la referencia anterior a `FrozenPage.jsx` y a 7 rutas ya retiradas quedó obsoleta el mismo día que se escribió): `public/src/pages/ModulePending.jsx`, usado hoy únicamente por `/ficha` (`public/src/App.jsx`) — legítimo mientras `/ficha` siga en desarrollo activo sin contrato de backend confirmado (ver `docs/ARQUITECTURA_AGENTICA_ANTIGRAVITY.md §0-AI`, "Corte de Perímetro"). Las 7 rutas que antes usaban este patrón (`/panel`, `/directorio`, `/favoritos`, `/calendario`, `/anexos`, `/logistica`, `/dialetica`) fueron retiradas del router — si reaparecen citando `ModulePending`, es una regresión real, no un falso positivo. Busca (Grep) el mismo patrón estructural en componentes nuevos o modificados: JSX sin `fetch`/`useEffect`/hook de datos, con contenido hardcodeado que aparenta ser dinámico.
