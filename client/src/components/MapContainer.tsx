...
export default function MapContainer() {
  ...
  // New useEffect to recenter map when selectedPredio changes
  useEffect(() => {
    if (!mapInstanceRef.current || !selectedPredio) return;

    const L = import('leaflet').then(L => {
      const lat = selectedPredio.lat ?? mapCenter.lat;
      const lng = selectedPredio.lng ?? mapCenter.lng;
      mapInstanceRef.current.setView([lat, lng], 16); // Zoom level 16 for detailed view
    });
  }, [selectedPredio, mapInstanceRef.current, mapCenter]);
