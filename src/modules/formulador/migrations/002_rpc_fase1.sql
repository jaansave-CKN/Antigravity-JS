-- =============================================================
-- Formulador AI — RPC function para insert atómico de Fase 1
-- Ejecutar en: Supabase Dashboard → SQL Editor
-- Permite llamar vía REST: POST /rest/v1/rpc/insertar_fase1
-- =============================================================
...
BEGIN
  ...
  INSERT INTO formulador_proyectos (
    ...
    p_ficha->>'enfoque',
    p_ficha->>'regimen',
    p_ficha->>'sector_codigo',
    p_ficha->>'clasificacion_infra',
    p_ficha->>'departamento',
    p_ficha->>'municipio',
    p_ficha->>'zona',
    p_ficha->>'diagnostico',
    (p_ficha->>'poblacion_total')::INTEGER,
    'draft'
  )
  ...
  INSERT INTO formulador_objetivos (
    tenant_id, proyecto_id, objetivo_general,
    og_indicador, og_meta, og_linea_base, cadena_valor
  ) VALUES (
    p_tenant_id, v_proyecto_id,
    p_modulo_7->>'objetivo_general',
    p_modulo_7->>'objetivo_general_indicador',
    p_modulo_7->>'objetivo_general_meta',
    p_modulo_7->>'objetivo_general_linea_base',
    COALESCE(p_modulo_7->'cadena_valor', '{}')
  )
  ...
  LOOP
    INSERT INTO formulador_oe (
      tenant_id, objetivo_id, proyecto_id, oe_numero,
      descripcion, indicador, meta, unidad, linea_base
    ) VALUES (
      p_tenant_id, v_objetivo_id, v_proyecto_id,
      (SELECT COUNT(*) FROM formulador_oe WHERE proyecto_id = v_proyecto_id) + 1,
      v_oe->>'descripcion', v_oe->>'indicador', v_oe->>'meta',
      v_oe->>'unidad', v_oe->>'linea_base'
    );
  END LOOP;

  -- 4. Cronograma
  INSERT INTO formulador_cronograma (
    tenant_id, proyecto_id, duracion_meses,
    fecha_inicio, fecha_fin, fases, hitos
  ) VALUES (
    p_tenant_id, v_proyecto_id,
    (p_modulo_8->>'duracion_meses')::SMALLINT,
    (p_modulo_8->>'fecha_inicio')::DATE,
    (p_modulo_8->>'fecha_fin')::DATE,
    COALESCE(p_modulo_8->'fases', '[]'),
    COALESCE(p_modulo_8->'hitos', '[]')
  );

  -- 5. Presupuesto
  INSERT INTO formulador_presupuesto (
    tenant_id, proyecto_id, presupuesto_total, moneda,
    fuentes, contrapartida_monetaria, contrapartida_especie,
    contrapartida_desc, resumen, viabilidad_financiera
  ) VALUES (
    p_tenant_id, v_proyecto_id,
  ...
  INSERT INTO formulador_validaciones_financieras (
    tenant_id, proyecto_id, estado,
    porcentaje_contrapartida_real, porcentaje_minimo_requerido,
    fuente_evaluada, mensaje
  ) VALUES (
    p_tenant_id, v_proyecto_id, v_estado,
    ROUND(v_pct_real, 2), v_regla_min,
  ...
  RETURN jsonb_build_object(
    ...
    'porcentaje_contrapartida', ROUND(v_pct_real, 2)
  );
END;
$$;
