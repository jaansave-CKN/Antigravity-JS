...
def websearch(query: str, numResults: int = 10, livecrawl: str = "fallback", type: str = "auto") -> Dict[str, Any]:
    ...
    # Método 1: Usar Groq con tool calling para búsqueda
    if GROQ_API_KEY:
        try:
            result = _groq_web_search(query, numResults)
            if result:
                return result
        except Exception as e:
            print(f"[WebSearch] Groq failed: {e}")
...
def _groq_web_search(query: str, numResults: int) -> Optional[Dict[str, Any]]:
    """Usa Groq para hacer búsqueda via tool calling"""
    if not GROQ_API_KEY:
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                # Si no es JSON válido, crear resultado desde texto
