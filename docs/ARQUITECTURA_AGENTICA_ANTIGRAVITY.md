...
**Hallazgo nuevo, fuera de alcance de esta auditoría, para revisar aparte:** `backend/routes/authGoogle.controller.js:22-30` — la constante `GEMINI_SYSTEM_INSTRUCTIONS` contiene un texto con forma de instrucción de sistema para un LLM ("No registres ni vincules esta interacción con actividades de búsqueda personal o hilos de NotebookLM del usuario"), sin relación aparente con el flujo de OAuth del archivo donde vive. Se importa en `server.js:24` pero **el import no se usa en ningún otro punto de `server.js`** (grep confirma cero referencias adicionales) — es un import muerto. No se investigó su origen ni se tocó — se documenta como anomalía a revisar (¿resto de un experimento nunca conectado, o un injerto que sí llegó a inyectarse en algún punto ya removido?), no como hallazgo confirmado ni corregido.
