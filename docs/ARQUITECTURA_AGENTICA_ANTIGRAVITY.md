...
- No se implementó el nonce firmado para el `state` de OAuth de Google (hallazgo #1 de §6.2) — requiere diseño propio, no es una corrección de una línea.
- No se agregó `aiLimiter` a los endpoints de radar masivo (§6.4) ni un tope server-side a `/api/ai/generate` — quedan documentados para una ronda de hardening de FinOps dedicada.
- No se investigó ni se borró `backend/core/*.py` (código muerto, §6.1) — se documenta su existencia, la decisión de qué hacer con él es del usuario.
- No se corrigió la atomicidad de `runTransaction()` en Capa 2 (§6.3, hallazgo 3) — es una decisión de diseño (¿aceptar el riesgo documentado, o bloquear escrituras multi-sentencia cuando Capa 2 está activa?), no un fix mecánico.
- No se configuró `ANTHROPIC_API_KEY` para activar el gate técnico (§6.5) — requiere que el usuario decida pagar por esa credencial, igual que Stripe/Wompi.
