...
| 2 | `iframe-missing-sandbox` (dashboard PostHog embebido) | `client/src/pages/AdminPage.tsx:161` (antes del fix) | **Real, mitigado parcialmente — no eliminado** — sin `sandbox`, el iframe tenía navegación de top-level y popups sin restricción. Con `sandbox` agregado, `react-doctor` sigue marcando la línea porque `allow-scripts`+`allow-same-origin` juntos (imprescindibles para que la sesión de PostHog funcione embebida) devuelven casi toda la capacidad que el sandbox existe para quitar — trade-off aceptado y documentado en el código, mismo patrón que cualquier dashboard SaaS embebido (Grafana/Metabase) | ✅ **Endurecido en esta pasada** — se quitan `allow-forms`/`allow-popups` (no eran necesarios); queda solo el par mínimo requerido, documentado inline como decisión consciente, no como pendiente sin investigar |
