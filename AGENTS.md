...
* **`001-orquestador-maestro`**: Enrutador Central y Cadena de Mando — `.claude/agents/001-orquestador-maestro.md`. Sin `Write`/`Edit`/`Bash` (retirados 2026-08-12: no puede autoaprobarse ni saltarse el gate) — delega todo vía `Agent`. (Nota 2026-08-16: `agents/001-orquestador-maestro/` en disco es Sistema A legacy con nombre idéntico tras la normalización de nomenclatura — no confundir, ver advertencia en su IDENTITY.md.)
