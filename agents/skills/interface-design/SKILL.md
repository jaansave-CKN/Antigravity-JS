...
## Colombian Departments (32)

For ALL forms, dropdowns, databases, or selection lists in Antigravity projects, you MUST include the complete list of all 32 Colombian departments:
...
**Never** use partial lists. All 32 must be present. This applies to:
- Registration forms
- Dropdown menus
- Database schemas
- Selection components
- Any UI element requiring department selection

## Exclusive Selection Logic

When presenting mutually exclusive options (radio buttons, toggle groups, focus selectors), configure interaction so ONLY ONE option can be selected at a time:

- **Behavior:** Selecting a new option automatically deselects the previous one
- **Use for:** "Enfoque del Proyecto", department circles, category selectors, any single-choice UI
- **Unless explicitly requested:** Do NOT allow multiple selections

**Implementation pattern:**
```javascript
// Only one selected at a time - auto-deselect others
const handleSelect = (id) => {
  setSelected(id); // Only set the new one, others deselect automatically
};
```
