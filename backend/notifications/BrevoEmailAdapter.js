...
export class BrevoEmailAdapter {
  ...
  async sendAccountActivatedNotice(userEmail, { nombre, validarUrl }) {
    return this.sendEmail({
      to:      userEmail,
      subject: '¡Tu cuenta ya fue validada! — RadFor-360',
      content: `
        <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;padding:2rem;">
          <h2 style="color:#191c1e;margin-bottom:0.5rem;">Tu cuenta ya fue validada</h2>
          <p style="color:#45464d;">Hola ${nombre}, el administrador aprobó tu cuenta en <strong>RadFor-360</strong>. Ya puedes entrar directo al portal con un clic.</p>
          <a href="${validarUrl}"
             style="display:inline-block;margin:1.5rem 0;padding:12px 28px;background:#16a34a;color:#fff;
                    text-decoration:none;border-radius:8px;font-weight:700;font-family:monospace;
                    letter-spacing:0.05em;text-transform:uppercase;font-size:12px;">
            Validar y entrar
          </a>
          <p style="color:#9ca3af;font-size:11px;margin-top:2rem;">
            Este enlace es de un solo uso e inicia sesión automáticamente. Si prefieres, también puedes iniciar sesión normalmente con tu correo y contraseña.
          </p>
          <hr style="border:0;border-top:1px solid #e5e7eb;margin:2rem 0;"/>
          <p style="color:#c6c6cd;font-size:10px;font-family:monospace;">RadFor-360 · Sistema de Formulación de Proyectos</p>
        </div>
      `,
    });
  }
