...
export async function ingestConvocatorias() {
  const report = {
    secop:     { inserted: 0, skipped: 0, errors: 0, errorMsg: '' },
    worldbank: { inserted: 0, skipped: 0, errors: 0, errorMsg: '' },
  ...
  try {
    ...
    for (const item of secopItems) {
      try {
        const urlConvocatoria = item.url_del_proceso || '';
        const titulo = (
          item.descripci_n_del_procedimiento ||
          item.objeto_del_proceso ||
          'PROCESO SIN TÍTULO'
        ).slice(0, 255);

        // Verificar duplicado por url_convocatoria o título + estado no eliminado
        const existing = await getRow(
          'SELECT id FROM convocatorias WHERE url_convocatoria = ? OR (titulo = ? AND deleted_at IS NULL)',
          [urlConvocatoria, titulo]
        );
        if (existing) { report.secop.skipped++; continue; }

        const refId = item.id_del_proceso || item.referencia_del_proceso || '';
        const monto = safeFloat(item.cuant_a_estudio_previo || item.valor_total_adjudicacion);

        await runSql(
          `INSERT INTO convocatorias
             (id, externo_id, titulo, donante, fuente, descripcion,
          ...
          [
            ...
            refId,
            titulo,
            item.nombre_entidad || 'SECOP II',
            'SECOP_II',
            `REF:${refId} | ENTIDAD:${item.nombre_entidad || ''} | MODALIDAD:${item.modalidad_de_contrataci_n || ''}`,
            0,
            monto,
            'COP',
            '["Colombia"]',
            ...
            item.fecha_de_inicio_del_proceso || '',
            '[]',
            'nueva',
            80,
    ...
    report.secop.errorMsg = e.message;
  }
  ...
  try {
    ...
    for (const p of wbProjects) {
      try {
        const urlConvocatoria = p.url || '';

        const existing = await getRow(
          'SELECT id FROM convocatorias WHERE url_convocatoria = ?',
          [urlConvocatoria]
        );
        if (existing) { report.worldbank.skipped++; continue; }

        const titulo = (p.project_name || 'Proyecto Banco Mundial').slice(0, 255);
        const monto  = safeFloat(p.totalamt);
        const sector = p.mjsector_namecode || 'DESARROLLO';
    ...
    report.worldbank.errorMsg = e.message;
  }
