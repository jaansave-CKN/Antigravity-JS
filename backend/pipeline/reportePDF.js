/**
 * F5-02 — Motor de Reportes PDF Certificados
 * Server-Side Rendering exclusivo. Protocol Precision: sin sombras, 0px borders, tipografía limpia.
 */

import PDFDocument from 'pdfkit';
import crypto from 'crypto';

// ── Protocol Precision palette ────────────────────────────────────────────────
const C = {
  primary:  '#1A1A2E',
  accent:   '#16213E',
  green:    '#198754',
...
/**
 * Hash SHA-256 que vincula proyecto + sello de auditoría.
 * Permite verificar la integridad del reporte ante entidades externas.
 */
...
/**
 * Genera el buffer PDF del reporte certificado.
  ...
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        size:        'A4',
        bufferPages: true,   // permite iterar páginas al final para el footer
        margins:     { top: 60, bottom: 80, left: 50, right: 50 },
        info: {
          Title:    `Reporte de Radicación — ${proyecto.nombre}`,
      ...
      // Split 64-char hash into two 32-char lines for readability
      ...
      doc.font(NORMAL).fontSize(7.5).fillColor(C.muted)
         .text(
           'Este documento fue generado mediante Server-Side Rendering exclusivo por el motor ' +
           'RadarFondos 360. El Hash de Certificación SHA-256 vincula matemáticamente el contenido ' +
           'de este reporte con el registro de auditoría almacenado en base de datos. ' +
           'Cualquier alteración al contenido invalida el hash, permitiendo detectar falsificaciones. ' +
           'Válido ante entidades certificadoras conforme al protocolo Precision F5-01.',
           { align: 'justify', lineGap: 1.5 }
         );

      // ── FOOTER EN TODAS LAS PÁGINAS ───────────────────────────────────────
      // bufferPages: true permite iterar y escribir en páginas ya renderizadas
...
function hRule(doc, ml, w, color, lw = 0.5) {
  const y = doc.y;
